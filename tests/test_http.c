...
void suite_http(void) {
    ...
    ph_strbuf_init(&out);
    r = ph_http_build_request("http://example.com/batch/", body, strlen(body), &out);
    CHECK(r == PH_OK);
    ...
    ph_strbuf_init(&out);
    r = ph_http_build_request("https://us.i.posthog.com/batch/", body, strlen(body), &out);
    CHECK(r == PH_ERR);
    ph_strbuf_free(&out);

    /* unsupported scheme */
    ph_strbuf_init(&out);
    r = ph_http_build_request("ftp://nope/", body, strlen(body), &out);
    CHECK(r == PH_ERR);
    ph_strbuf_free(&out);
}
