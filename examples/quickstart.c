 ...
 * PostHog host - HTTPS works on Windows (WinHTTP), macOS (Secure Transport), and
 * Linux (system OpenSSL). With nothing listening on the dev proxy below, the
 * sender logs a failed POST and drops the batch - which is what on_log prints.
 */
#include "posthog.h"
